...
def resnet101(bn_momentum=0.1, pretrained=False, output_stride=16):
    ...
    if pretrained:
        model = load_pretrained_model(model, 'resnet101')
    return model
def resnet152(bn_momentum=0.1, pretrained=False, output_stride=16):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    ckpt = './resnet152-b121ed2d.pth'
    model = ResNet(Bottleneck, [3, 8, 36, 3], bn_momentum, pretrained, output_stride)
    if pretrained:
        model = load_pretrained_model(model, 'resnet152')
    return model
...
def load_pretrained_model(model, model_name):
    ...
    # pretrain_dict = torch.load(ckpt[model_name])
